...
def train(epoch):
    ...
    labels_np = min_max_scalers[-1].inverse_transform(labels.data.cpu().numpy())
    loss_val = np.sqrt(mean_squared_error(output_np[idx_val.cpu().numpy()], labels_np[idx_val.cpu().numpy()]))
    loss_train = np.sqrt(mean_squared_error(output_np[idx_train.cpu().numpy()], labels_np[idx_train.cpu().numpy()]))
